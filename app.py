...
def get_gemini_response(question, prompt, model_name):
    """Generate SQL query using a supported Gemini model"""
    ...
    while True:
        try:
            ...
            st.warning("🚨 Gemini API quota exceeded. Waiting 60 seconds...")
            time.sleep(60)
        except Exception as e:
            return f"❌ Error: {e}"
...
# Updated Prompt Template for Your Latest STUDENT Database
prompt_template = """
You are an expert AI converting English questions into SQL queries.
...
2. Never include backticks or Markdown.
3. Use correct column names.
4. Use WHERE for filtering.
5. Use COUNT(), AVG(), MAX(), ORDER BY, LIMIT when required.
6. Important: DO NOT guess columns not in the table.
...
Q: List students from AI department.
SQL: SELECT * FROM STUDENT WHERE DEPARTMENT='AI';

Q: Show students with CGPA above 3.5.
SQL: SELECT * FROM STUDENT WHERE CGPA > 3.5;

Q: Get names of students from semester 2.
SQL: SELECT NAME FROM STUDENT WHERE SEMESTER = 2;
"""

# Submit Button
submit_button = st.button("Generate SQL & Execute")

if submit_button and question:
    with st.spinner("Finding supported model..."):
